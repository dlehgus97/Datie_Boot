...
public class PayController {
    ...
    public ResponseEntity<String> handlePayment(@RequestBody PaymentRecord paymentRecord,@RequestParam String key) {

        // processPayment 메서드를 호출하여 결과를 얻음
        String result = paymentRecordService.processPayment(paymentRecord);
        String keyresult = paymentKeyService.paymentkeyUsed(key);
        // 결과에 따라 적절한 HTTP 상태 코드와 응답 본문을 설정하여 반환
        if("Key 사용".equals(keyresult)){
